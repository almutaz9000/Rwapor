...
## 2026-06-24 - [Vectorized Dekad Table Generation]
**Learning:** In date-sequence and dekadal interval planning operations, generating dekads using an iterative `while` loop with per-day date string formatting, individual `as.Date()` calls, and `do.call(rbind, ...)` single-row data frame binding creates severe memory reallocation and R-level loop overhead. Generating monthly sequence bounds (`seq.Date`), constructing atomic start/end day vectors in bulk, and clamping with vectorized `pmax`/`pmin` (wrapped with `structure(..., class = "Date")` to preserve S3 Date attributes) converts dekad table construction to a single-pass $O(1)$ vectorized operation.
**Action:** Replace iterative date interval loops and single-row `rbind` calls with bulk date sequence generation and vectorized `pmax`/`pmin` atomic vector construction.
