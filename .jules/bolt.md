...
## 2025-05-18 - [Efficient Frequency Counting with crosstab]
**Learning:** For generating unique combinations of values from multiple rasters (e.g., seasonal profiles), using `terra::crosstab(..., long = TRUE)` is significantly more memory-efficient than extracting all pixels into R via `terra::values()` and using `stats::aggregate()`. It executes the grouping and counting in the C++ backend, avoiding the overhead of creating massive intermediate data frames in R.
**Action:** Use `terra::crosstab(..., long = TRUE)` for spatial frequency analysis and unique combination extraction.
