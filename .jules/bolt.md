...
## 2025-05-18 - [Vectorized Date Parsing and Unit Conversion Factors]
**Learning:** Iteratively parsing date information from URLs using `lapply(urls, wapor_date_info)` and calculating conversion factors via `vapply(..., calculate_conversion_factor)` introduces significant R-level overhead, especially for long time series (e.g., several years of daily data). Vectorizing these utilities using R's native vector operations (e.g., `substr()`, `paste()`, and vectorized `switch` equivalents) drastically reduces the overhead of metadata preparation and temporal unit conversion.
**Action:** Always provide vectorized utility functions for metadata parsing and parameter calculation to avoid $O(N)$ R-level loop overhead in data pipelines.
