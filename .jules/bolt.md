...
## 2024-05-16 - [Vectorized Zonal Stats Reshaping]
...
## 2026-06-23 - [Subscript Mixing in R Vectorized Indexing]
**Learning:** When vectorizing index-based cumulative sum operations in R, negative indexes do not return `NA` or `0` as in some other languages; instead, they act as element exclusions, altering the shape of the indexing vector. Furthermore, mixing positive and negative indexes throws a fatal `only 0's may be mixed with negative subscripts` error.
**Action:** Always clamp vectorized indexing arrays to safe positive integers (e.g., using `pmax(1L, pmin(max_length, indices))`) to prevent mixing and exclusion bugs before indexing R cumulative sum arrays.
