...
## 2025-05-18 - [Optimization of Zonal Quantiles]
**Learning:** Using custom R closures for quantile calculation in `terra::zonal` is a significant bottleneck because it forces `terra` to drop out of its optimized C++ backend for every zone. Using the built-in string identifier `"quantile"` (with `probs` and `na.rm` passed as additional arguments) allows the operation to run entirely in C++.
**Action:** Always use built-in string identifiers (mean, sum, quantile, etc.) in `terra::zonal` and `terra::global` instead of custom R functions. If conditional logic (like a minimum pixel threshold) is needed, implement it as a vectorized post-processing step on the resulting data frame.
