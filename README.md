...
> temporal resolutions of a variable — dekadal (`-D`), monthly (`-M`), and
> annual (`-A`) products share the same native spatial resolution as their
> `-E`/`-D` counterpart shown here (e.g. `L1-AETI-M` and `L1-AETI-A` are
> still ~300 m, `L1-PCP-M` and `L1-PCP-A` are still ~5 km).
