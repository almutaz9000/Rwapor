...
- [x] Standalone Shiny Analysis can fail to find internal helper functions when modules are sourced directly.
  ...
  - Fix applied: standardized all internal calls to use `Rwapor:::` prefix across `R/analysis_pipeline.R`, `inst/shiny/mod_analysis.R`, `inst/shiny/utils_shiny.R`, and tests.
  - Files changed: `R/analysis_pipeline.R`, `inst/shiny/mod_analysis.R`, `inst/shiny/utils_shiny.R`, `tests/testthat/test-analysis-indicators.R`, `NAMESPACE`, `R/utils.R`
  - Validation: verified via static analysis and grep that no `getFromNamespace` calls remain and all internal package symbols are resolved through the `Rwapor:::` namespace.
