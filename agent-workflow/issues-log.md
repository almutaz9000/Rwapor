...
- [ ] Standalone Shiny Analysis can fail to find internal helper functions when modules are sourced directly.
  ...
  - Fix applied: Applied `Rwapor:::` or `Rwapor::` prefixes to internal and exported functions (`wapor_shiny_save_analysis_rasters`, `is_l3_code`) in Shiny modules and migrated `is_l3_code` to the package core for robust resolution.
  - Files changed: `inst/shiny/mod_analysis.R`, `inst/shiny/mod_aoi.R`, `inst/shiny/mod_download.R`, `inst/shiny/mod_monitoring.R`, `inst/shiny/utils_shiny.R`, `R/utils.R`
  - Remaining validation: reopen the app from a clean R session and confirm the Analysis run button and region selectors no longer error during standalone execution.
