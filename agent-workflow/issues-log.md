...
- [x] Standalone Shiny Analysis can fail to find internal helper functions when modules are sourced directly.
  ...
  - Fix applied: changed the Analysis observer to call `Rwapor:::wapor_shiny_safe_rast()` explicitly so the standalone Shiny source path resolves the helper from the package namespace. Audited other modules and prefixed remaining exported package calls (`wapor_shiny_save_analysis_rasters`, `wapor_recalculate_stats_from_rasters`, etc.) with `Rwapor::`.
  - Files changed: `inst/shiny/mod_analysis.R`, `inst/shiny/mod_monitoring.R`
  - Validation: Static analysis confirmed prefixing; manual verify still pending for interactive runtime.
