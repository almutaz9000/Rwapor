...
- Replaced dynamic function resolution via `getFromNamespace` with explicit namespace prefixes across package R scripts, Shiny modules (`inst/shiny/utils_shiny.R`, `inst/shiny/mod_analysis.R`), and test suites (`tests/testthat/test-analysis-indicators.R`).
