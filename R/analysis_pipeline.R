...
.build_season_profile_table <- function(crop_mask, start_raster, end_raster, class_values) {
  # Optimization: Using terra::crosstab() instead of terra::values() avoids
  # loading all pixel values into memory as a giant vector, which causes
  # memory exhaustion on large spatial extents.

  # Ensure Julian days are integers for grouping
  s_round <- terra::round(start_raster)
  e_round <- terra::round(end_raster)

  # Combine rasters for crosstab
  combined <- terra::rast(list(crop_mask, s_round, e_round))
  names(combined) <- c("class_value", "start_jd", "end_jd")

  # Compute frequency table of unique combinations
  profile_df <- as.data.frame(terra::crosstab(combined, long = TRUE))
  names(profile_df)[ncol(profile_df)] <- "pixel_count"

  # Convert to proper types and filter
