...
wapor_calc_green_water <- function(aeti_seasonal, peff_seasonal) {
  # Optimization: pmin() dispatches to terra::pmin() for SpatRaster objects and base pmin for numeric.
  # Calling pmin directly avoids conditional branch evaluation (terra::ifel) in C++.
...
wapor_calc_blue_water <- function(aeti_seasonal, peff_seasonal) {
  ...
  if (inherits(diff_val, "SpatRaster")) {
    # Optimization: Using terra::clamp(..., lower = 0) leverages terra's native C++ clamping
    # implementation instead of conditional evaluation via terra::ifel.
