...
#'
#' Calculates seasonal crop evapotranspiration (ETc) by aggregating dekadal RET
#' weighted by season-fraction and crop coefficient (Kc). Pixels with partial
#' temporal coverage are summed ignoring NA values (na.rm = TRUE).
#'
#' @param ret_dekad SpatRaster. Dekadal RET layers.
#' @param season_weights SpatRaster. Dekadal season weights (0-1).
...
#' @param layer_multipliers Optional numeric vector of per-layer multipliers.
#' @param incremental Logical. If TRUE, performs aggregation layer-by-layer to save memory.
#'   Default FALSE (uses faster vectorized aggregation).
...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  if (incremental) {
    ...
    for (i in seq_len(n_layers)) {
      # Accumulate: term = RET_i * (weight_i * Kc_i * mult_i)
  ...
  # Optimization: Vectorized multiplication and specialized C++ sum is significantly
  # faster than R-level loops.
  # term = RET * SeasonWeights * (Kc * Multiplier)
  combined_mult <- kc_dekad * layer_multipliers
  weighted <- ret_dekad * (season_weights * combined_mult)
