...
#'
#' Computes seasonal ETc (weighted sum of RET * Kc) using a vectorized path.
#' This is significantly faster than iterative loops for long seasons.
#'
#' @param ret_dekad SpatRaster. Dekadal RET layers.
#' @param season_weights SpatRaster. Dekadal season weights (0-1).
...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  # Optimization: Use wapor_masked_sum which provides a vectorized terra::sum()
  # path (fast) and an optional incremental path (memory-safe).
  # We combine Kc and layer_multipliers into a single vector of weights.
  if (is.null(layer_multipliers)) {
