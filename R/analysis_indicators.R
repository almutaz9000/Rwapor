...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
                                       min_pixels = 30L) {
  # Optimization: Use built-in C++ implementation for quantile calculation.
  # This is significantly faster than passing a custom R closure to zonal().
  p95_vals <- terra::zonal(aeti_seasonal, crop_mask, fun = "quantile", probs = 0.95, na.rm = TRUE)
  p95_vals <- as.data.frame(p95_vals)

  # Optimization: Use built-in 'notNA' for counting valid pixels.
  # This avoids creating an intermediate ifel() raster and uses the C++ backend.
  count_vals <- terra::zonal(aeti_seasonal, crop_mask, fun = "notNA")
  count_vals <- as.data.frame(count_vals)
  
  # Standardize column names for robust merging across different terra versions
  ...
  # Optimization: Apply min_pixels threshold post-aggregation.
  # This keeps the aggregation phase fully vectorized in C++.
