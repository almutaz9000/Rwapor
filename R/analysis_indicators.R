...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  # Combine multipliers (Kc * overlap_days)
  lm <- if (is.null(layer_multipliers)) rep(1, n_layers) else layer_multipliers

  # Optimization: Delegate to wapor_masked_sum which uses vectorized terra::sum
  # and handles the incremental flag for memory efficiency.
  wapor_masked_sum(
    ret_dekad,
    season_weights,
    layer_multipliers = kc_dekad * lm,
    incremental = incremental
  )
}
