...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
  ...
  # faster than an R closure because it avoids the overhead of executing
  # conditional logic and subsetting for every zone. We calculate the 95th
  # percentile and then apply the pixel-count threshold in a vectorized
  # post-processing step.

  # To safely handle zones with only NAs (which cause stats::quantile to error),
  # we provide a wrapper that returns NA instead of erroring.
  safe_quantile <- function(x, ...) {
    if (length(x) == 0) return(NA_real_)
    stats::quantile(x, ...)
  }

  p95_vals <- terra::zonal(aeti_seasonal, crop_mask, fun = safe_quantile,
                           probs = 0.95, na.rm = TRUE)

  # Count valid analysis pixels per zone (non-NA in AETI).
  # terra::zonal does not support "notNA", so we use a binary mask + sum.
  ...
  # Merge stats and counts explicitly by zone column name
  zone_col <- names(p95_vals)[1]
  result <- merge(p95_vals, count_vals, by = zone_col, all.x = TRUE)
  
  # Standardize names and cast types for downstream compatibility
  # Ensure p95_aeti and n_pixels are explicitly named
  names(result) <- c("class_value", "p95_aeti", "n_pixels")
  ...
  # Vectorized validity check: enforce min_pixels threshold and handle NAs
  # Using which() for robust subsetting if merged NAs are present.
