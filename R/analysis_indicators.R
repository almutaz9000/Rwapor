...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  if (!is.null(layer_multipliers) && length(layer_multipliers) != n_layers) {
    stop(sprintf("layer_multipliers length (%d) must match ret_dekad layers (%d)",
                 length(layer_multipliers), n_layers), call. = FALSE)
  }

  # Optimization: Use the vectorized wapor_masked_sum helper which replaces
  # the iterative R-level loop with the specialized C++ backend (terra::sum).
  # Combine Kc and other rate multipliers into a single weight vector.
  multipliers <- kc_dekad * (layer_multipliers %||% rep(1, n_layers))
