...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
                                       min_pixels = 30L) {
  # Optimization: Use built-in string identifiers in terra::zonal to leverage
  # the C++ backend for single-pass processing.
  res <- terra::zonal(aeti_seasonal, crop_mask, fun = c("quantile", "notNA"),
                      probs = 0.95, na.rm = TRUE)
  res <- as.data.frame(res)

  # Standardize names: [Zone, Quantile, notNA]
  # Depending on terra version, column names vary; use indexing for robustness.
  names(res)[1:3] <- c("class_value", "p95_aeti", "n_pixels")
  
  result <- data.frame(
    class_value = as.integer(res$class_value),
    p95_aeti    = as.numeric(res$p95_aeti),
    n_pixels    = as.integer(res$n_pixels),
    stringsAsFactors = FALSE
  )
  
  # Apply min_pixels threshold
