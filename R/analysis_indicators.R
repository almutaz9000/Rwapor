...
wapor_calc_green_water <- function(aeti_seasonal, peff_seasonal) {
  # Optimization: pmin dispatches to terra::pmin for SpatRaster or base pmin for numeric,
  # avoiding conditional branch evaluation overhead of terra::ifel.
...
wapor_calc_blue_water <- function(aeti_seasonal, peff_seasonal) {
  ...
  if (inherits(diff_val, "SpatRaster")) {
    # Optimization: terra::clamp(..., lower = 0) is implemented directly in C++
    # and avoids conditional branch evaluation overhead of terra::ifel.
