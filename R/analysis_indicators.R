...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  # Optimization: Delegate summation to wapor_masked_sum.
  # This provides a vectorized path for performance and an incremental path
  # for memory efficiency while deduplicating aggregation logic.
  if (is.null(layer_multipliers)) {
    layer_multipliers <- rep(1, n_layers)
  }

  # Combine Kc values with temporal multipliers (e.g., n_days)
...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
                                       min_pixels = 30L) {
  # Optimization: Use built-in "quantile" string identifier in terra::zonal.
  # This executes the quantile calculation in the C++ backend, which is
  # significantly faster than passing an R closure.
  p95_vals <- terra::zonal(aeti_seasonal, crop_mask, fun = "quantile",
                          probs = 0.95, na.rm = TRUE)

  # Count valid analysis pixels per zone.
  # terra::zonal does not support "notNA" string identifier, so we use a binary mask.
  ...
  # Standardize result column names
  ...
  # Post-process: Enforce the min_pixels threshold and ensure integer types.
  # Using logical indexing for efficiency on merged results.
  result$n_pixels <- as.integer(result$n_pixels)
  result$p95_aeti <- ifelse(result$n_pixels < min_pixels, NA_real_, as.numeric(result$p95_aeti))
