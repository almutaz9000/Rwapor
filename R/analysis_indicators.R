...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
                                       min_pixels = 30L) {
  # Optimization: Use built-in terra::zonal strings ("quantile", "notNA")
  # instead of R closures for significant performance gains (executed in C++).

  # 1. Compute 95th percentile
  ...
  # 2. Count non-NA pixels (built-in "notNA" is much faster than ifel + sum)
  ...
  # Standardize types and apply threshold in vectorized R
  ...
  # Mask out values with insufficient pixels
  result$p95_aeti[which(result$n_pixels < min_pixels)] <- NA_real_
