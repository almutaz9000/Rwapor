...
#'
#' Computes seasonal ETc by aggregating RET * season_weight * Kc.
#' Uses a vectorized approach for performance and supports an incremental
#' mode for memory efficiency.
#'
#' @param ret_dekad SpatRaster. Dekadal RET layers.
#' @param season_weights SpatRaster. Dekadal season weights (0-1).
...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  # Optimization: Use the vectorized wapor_masked_sum helper.
  # We combine the season weights (SpatRaster) and Kc (numeric vector) into
  # a single weight stack. Multiplying a SpatRaster by a numeric vector
  # is highly optimized in terra.
  combined_weights <- season_weights * kc_dekad
