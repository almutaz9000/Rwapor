...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
  ...
  # Optimization: Using the built-in "quantile" string is significantly faster
  # than a custom R closure as it executes in the C++ backend.
  ...
  names(count_vals)[seq_len(min(2, ncol(count_vals)))] <-
    c("class_value", "n_pixels")[seq_len(min(2, ncol(count_vals)))]
  ...
  # Merge pixel counts
  result <- merge(result, count_vals[, c("class_value", "n_pixels")],
                  by = "class_value", all.x = TRUE)
  result$n_pixels <- as.integer(result$n_pixels)
  result$n_pixels[is.na(result$n_pixels)] <- 0L

  # Apply min_pixels constraint: if count is too low, P95 is invalid (NA)
  # This maintains compatibility with the previous R-closure behavior.
