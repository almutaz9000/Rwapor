...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
                                       min_pixels = 30L) {
  # Optimization: Use built-in string identifiers in terra::zonal (C++ backend)
  # instead of slow R closures. Pass both functions for single-pass processing.
  # "notNA" is faster than ifel(is.na(x), 0, 1) + sum().
  res <- terra::zonal(aeti_seasonal, crop_mask, fun = c("quantile", "notNA"),
                      probs = 0.95, na.rm = TRUE)

  # Standardize output data frame
  result <- as.data.frame(res)
  # Ensure robust naming regardless of terra version/layer name
  names(result)[1:3] <- c("class_value", "p95_aeti", "n_pixels")
  ...
  # Apply min_pixels threshold and valid flag
  result$valid <- !is.na(result$p95_aeti) & result$n_pixels >= min_pixels
  # Reset p95 to NA for classes below threshold
