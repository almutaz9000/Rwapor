...
wapor_masked_sum <- function(x, weights, layer_multipliers = NULL, incremental = FALSE) {
  ...
  # Use na.rm = FALSE to maintain consistency with strict NA-propagation in R loops
...
#'
#' Calculates seasonal crop evapotranspiration (ETc) by aggregating dekadal
#' RET, season weights, and crop coefficients.
#'
#' @param ret_dekad SpatRaster. Dekadal RET layers.
#' @param season_weights SpatRaster. Dekadal season weights (0-1).
...
#'   to save memory. Recommended for very long seasons or low RAM. Default FALSE.
...
wapor_calc_seasonal_etc <- function(ret_dekad, season_weights, kc_dekad,
  ...
  if (incremental) {
    ...
    for (i in seq_len(n_layers)) {
      # term = RET_i * weight_i * (Kc_i * multiplier_i)
  ...
  # Vectorized path: significantly faster but uses more peak memory/disk
  # Compute combined weights first (scalar * scalar * SpatRaster)
  combined_weights <- season_weights * (kc_dekad * layer_multipliers)
  weighted_stack <- ret_dekad * combined_weights

  # Optimization: terra::sum() is faster than app(..., fun="sum")
  # Use na.rm = FALSE to maintain consistency with strict NA-propagation in R loops
  terra::sum(weighted_stack, na.rm = FALSE)
}
