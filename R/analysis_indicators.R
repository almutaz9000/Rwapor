...
wapor_calc_p95_aeti <- function(aeti_seasonal, crop_mask,
                                       min_pixels = 30L) {
  # Fast grouped quantile calculation using built-in terra::zonal "quantile".
  # Built-in functions are significantly faster than R closures.
  p95_vals <- as.data.frame(terra::zonal(aeti_seasonal, crop_mask, fun = "quantile", probs = 0.95, na.rm = TRUE))
  names(p95_vals)[1:2] <- c("class_value", "p95_aeti")

  # Count valid analysis pixels using built-in "notNA"
