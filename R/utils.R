...
#'
#' Efficiently parses a vector of WaPOR or AgERA5 raster URLs to extract
#' date information using vectorized string operations.
#'
...
wapor_parse_dates <- function(urls, tres) {
  ...
  # Extract the last dot-separated part (the date component)
  # Vectorized approach: use sub() to keep only the part after the last dot.
  ...
  if (tres == "D") {
    # Handles both YYYY-MM-DX and YYYYMMDD
    is_compact <- nchar(date_components) == 8 & grepl("^\\d{8}$", date_components)

    # Process compact format (YYYYMMDD)
    if (any(is_compact)) {
      comp <- date_components[is_compact]
      year_str <- substr(comp, 1, 4)
      month_str <- substr(comp, 5, 6)
      day_val <- as.numeric(substr(comp, 7, 8))

      start_dates[is_compact] <- paste(year_str, month_str, sprintf("%02d", day_val), sep = "-")

      # Determine dekad for end date calculation
      dekad_type <- ifelse(day_val <= 10, 1L, ifelse(day_val <= 20, 2L, 3L))
      ...
      end_dates[is_compact] <- ifelse(
        ...
        ifelse(
          ...
          # D3: use lubridate for last day of month
          as.character(lubridate::ceiling_date(lubridate::ymd(start_dates[is_compact]), "month") - 1)
        )
      )
    }

    # Process standard format (YYYY-MM-DX)
    if (any(!is_compact)) {
      std <- date_components[!is_compact]
      parts <- strsplit(std, "-", fixed = TRUE)

      # Validate part count
      valid_parts <- vapply(parts, length, integer(1)) == 3
      if (any(!valid_parts)) {
        stop(sprintf("Cannot parse date from URL for Dekadal data: %s", filenames[!is_compact][!valid_parts][1]), call. = FALSE)
      }
      
      year_str <- vapply(parts, `[`, 1, FUN.VALUE = character(1))
      month_str <- vapply(parts, `[`, 2, FUN.VALUE = character(1))
      dekad_str <- vapply(parts, `[`, 3, FUN.VALUE = character(1))
      
      dekad_map_start <- c("D1" = "01", "D2" = "11", "D3" = "21", "1" = "01", "2" = "11", "3" = "21")
      start_day <- dekad_map_start[dekad_str]

      if (any(is.na(start_day))) {
         stop(sprintf("Unknown dekad format: %s", dekad_str[is.na(start_day)][1]), call. = FALSE)
      }
      
      start_dates[!is_compact] <- paste(year_str, month_str, start_day, sep = "-")
    ...
    # Monthly: YYYY-MM
    parts <- strsplit(date_components, "-", fixed = TRUE)
    if (any(vapply(parts, length, integer(1)) < 2)) {
      stop(sprintf("Cannot parse date from URL for Monthly data: %s", filenames[vapply(parts, length, integer(1)) < 2][1]), call. = FALSE)
    }
    year_str <- vapply(parts, `[`, 1, FUN.VALUE = character(1))
    month_str <- vapply(parts, `[`, 2, FUN.VALUE = character(1))
    start_dates <- paste(year_str, month_str, "01", sep = "-")
    end_dates <- as.character(lubridate::ceiling_date(lubridate::ymd(start_dates), "month") - 1)
  ...
  ndays <- as.numeric(difftime(end_dt, start_dt, units = "days")) + 1
...
#' Extract Date Information from URL (Backward Compatible)
#'
#' @param url Character scalar.
#' @param tres Character. "D", "M", "A", or "E".
#' @return A list.
#' @export
