...
#'
#' Optimized vectorized version of wapor_date_info for processing multiple URLs
#' in a single pass.
#'
#' @param urls Character vector of resource URLs or filenames.
...
wapor_parse_dates <- function(urls, tres) {
  ...
  # Extract date component: the last dot-separated part before extension
  # e.g., "WAPOR-3.L1-AETI-D.2023-01-D1.tif" -> "2023-01-D1"
  bases <- sub("\\.[^.]+$", "", filenames)
  ...
  if (tres == "D") {
    ...
    n_dash <- lengths(dash_parts)

    # Handle YYYY-MM-DX
    is_dash <- n_dash >= 3
    if (any(is_dash)) {
      y_str <- vapply(dash_parts[is_dash], `[`, 1, FUN.VALUE = character(1))
      m_str <- vapply(dash_parts[is_dash], `[`, 2, FUN.VALUE = character(1))
      d_str <- vapply(dash_parts[is_dash], `[`, 3, FUN.VALUE = character(1))

      dekad_map <- c("D1" = "01", "D2" = "11", "D3" = "21", "1" = "01", "2" = "11", "3" = "21")
      if (!all(d_str %in% names(dekad_map))) {
         stop(sprintf("Unknown dekad format in URLs: %s", paste(unique(d_str[!d_str %in% names(dekad_map)]), collapse=", ")), call. = FALSE)
      }
      
      start_dates[is_dash] <- paste(y_str, m_str, dekad_map[d_str], sep = "-")

      # End dates
      e_days <- rep("10", length(y_str))
      e_days[d_str %in% c("D2", "2")] <- "20"
      
      is_d3 <- d_str %in% c("D3", "3")
      if (any(is_d3)) {
        dt_objs <- lubridate::ymd(start_dates[is_dash][is_d3])
        e_days[is_d3] <- as.character(lubridate::days_in_month(dt_objs))
      }
      end_dates[is_dash] <- paste(y_str, m_str, e_days, sep = "-")
    }

    # Handle YYYYMMDD
    is_compact <- !is_dash & nchar(date_components) == 8 & grepl("^\\d{8}$", date_components)
    if (any(is_compact)) {
      comp <- date_components[is_compact]
      y_str <- substr(comp, 1, 4)
      m_str <- substr(comp, 5, 6)
      d_val <- as.numeric(substr(comp, 7, 8))
      start_dates[is_compact] <- paste(y_str, m_str, sprintf("%02d", d_val), sep = "-")

      e_days <- rep("10", length(y_str))
      e_days[d_val > 10 & d_val <= 20] <- "20"
      
      is_d3 <- d_val > 20
      if (any(is_d3)) {
        dt_objs <- lubridate::ymd(start_dates[is_compact][is_d3])
        e_days[is_d3] <- as.character(lubridate::days_in_month(dt_objs))
      }
      end_dates[is_compact] <- paste(y_str, m_str, e_days, sep = "-")
    }

    if (any(!is_dash & !is_compact)) {
       stop(sprintf("Cannot parse dekadal dates from filenames: %s", paste(filenames[!is_dash & !is_compact], collapse=", ")), call. = FALSE)
    }

  } else if (tres == "M") {
    # Monthly format: YYYY-MM
    dash_parts <- strsplit(date_components, "-")
    if (any(lengths(dash_parts) < 2)) {
      stop("Cannot parse monthly dates from filenames", call. = FALSE)
    }
    y_str <- vapply(dash_parts, `[`, 1, FUN.VALUE = character(1))
    m_str <- vapply(dash_parts, `[`, 2, FUN.VALUE = character(1))
    start_dates <- paste(y_str, m_str, "01", sep = "-")
    dt_objs <- lubridate::ymd(start_dates)
    end_dates <- paste(y_str, m_str, lubridate::days_in_month(dt_objs), sep = "-")
    ...
    dash_parts <- strsplit(date_components, "-")
    if (any(lengths(dash_parts) < 3)) {
      stop("Cannot parse daily dates from filenames", call. = FALSE)
    }
