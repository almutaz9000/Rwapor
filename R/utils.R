...
#' Extract Date Information from URLs (Vectorized)
#'
#' Parses multiple WaPOR or AgERA5 raster filenames to extract date information.
#' This is significantly faster than calling \code{wapor_date_info} in a loop.
#'
...
#'   * `raw_date`: The original date component from filename
#'
#' @export
...
wapor_parse_dates <- function(urls, tres) {
  ...
  # Extract date component: part between last dot and extension
  # e.g., "WAPOR-3.L1-AETI-D.2023-01-D1.tif" -> "2023-01-D1"
  date_components <- sub("^.*\\.([^.]+)\\.[^.]+$", "\\1", filenames)
  ...
  if (tres == "D") {
    ...
    if (any(is_compact)) {
      ...
      days <- substr(date_components[is_compact], 7, 8)
      start_dates[is_compact] <- paste(years, months, days, sep = "-")
      
      # End dates for dekads
      day_vals <- as.numeric(days)
      end_days <- ifelse(day_vals <= 10, "10", ifelse(day_vals <= 20, "20", NA))
      
      # For D3, we need days_in_month
      d3_mask <- is.na(end_days)
      if (any(d3_mask)) {
        sd_obj <- lubridate::ymd(start_dates[is_compact][d3_mask])
        end_days[d3_mask] <- as.character(lubridate::days_in_month(sd_obj))
      }
      end_dates[is_compact] <- paste(years, months, end_days, sep = "-")
    }

    # Standard format YYYY-MM-DX
    if (any(!is_compact)) {
      dc_std <- date_components[!is_compact]
      parts <- strsplit(dc_std, "-")

      years <- vapply(parts, `[`, 1, FUN.VALUE = character(1))
      months <- vapply(parts, `[`, 2, FUN.VALUE = character(1))
      dekad_strs <- vapply(parts, `[`, 3, FUN.VALUE = character(1))

      dekad_map <- c("D1" = "01", "D2" = "11", "D3" = "21", "1" = "01", "2" = "11", "3" = "21")
      start_days <- dekad_map[dekad_strs]
      start_dates[!is_compact] <- paste(years, months, start_days, sep = "-")

      end_days <- ifelse(dekad_strs %in% c("D1", "1"), "10",
                         ifelse(dekad_strs %in% c("D2", "2"), "20", NA))
    ...
    # YYYY-MM
    years <- sub("^(\\d{4})-.*$", "\\1", date_components)
    months <- sub("^.*-(\\d{2})$", "\\1", date_components)
