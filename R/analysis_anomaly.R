...
wapor_detect_aeti_anomalies <- function(aeti_seasonal, crop_mask, 
  ...
  if (length(invalid_classes) > 0) {
    # OPTIMIZATION: Use terra's vectorized %in% operator to replace the iterative
    # R-level loop of ifel(). This executes the entire class masking in a single C++ pass.
