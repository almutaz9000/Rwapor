...
wapor_convert_raster <- function(r, variable, urls, unit_conversion) {
  ...
  # Compute all per-layer conversion factors up front (Vectorized)
  # terra broadcasts a numeric vector of length nlyr(r) element-wise across layers,
  # so the multiplication below triggers a single read pass.
  date_infos <- wapor_parse_dates(urls, tres)
  parsed_dates <- lubridate::ymd(date_infos$start_date)
  years <- as.integer(format(parsed_dates, "%Y"))
