...
wapor_convert_raster <- function(r, variable, urls, unit_conversion) {
  ...
  # Compute all per-layer conversion factors up front (one vectorized date parse
  # for all URLs, no repeated raster reads). terra broadcasts a numeric vector
  # of length nlyr(r) element-wise across layers, so the multiplication below
  # triggers a single read pass instead of nlyr separate passes.
