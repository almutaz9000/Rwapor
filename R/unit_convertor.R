...
wapor_convert_raster <- function(r, variable, urls, unit_conversion) {
  ...
  # Compute all per-layer conversion factors up front (vectorized date parse).
  # terra broadcasts a numeric vector of length nlyr(r) element-wise across layers,
  # so the multiplication below triggers a single read pass instead of nlyr
  # separate passes.
  date_infos <- wapor_parse_dates(urls, tres)
  sd_objs <- lubridate::ymd(date_infos$start_date)
  yr_vals <- as.integer(format(sd_objs, "%Y"))
  ...
  factors <- calculate_conversion_factor(
    ...
    lubridate::days_in_month(sd_objs),
    days_in_year = ifelse(lubridate::leap_year(yr_vals), 366L, 365L)
  )
