...
wapor_build_season_profile_table <- function(crop_mask, start_raster, end_raster, class_values) {
  ...
  # memory-efficient than extracting all pixels into R via terra::values() and
  # using stats::aggregate(). It executes grouping and counting in the C++ backend.

  # Round to integers for efficient crosstab and consistency with existing behavior
  s <- terra::round(start_raster)
  e <- terra::round(end_raster)

  # Combine rasters for crosstab (mask, start, end)
  # crosstab with long=TRUE returns a data.frame of unique combinations and their counts
  stk <- c(crop_mask, s, e)
  profile_df <- tryCatch({
  ...
  # Standardize names (crosstab names columns based on layers)
  names(profile_df) <- c("class_value", "start_jd", "end_jd", "pixel_count")

  # Filter for requested classes and compute duration
