...
wapor_build_season_profile_table <- function(crop_mask, start_raster, end_raster, class_values) {
  ...
  # to avoid memory exhaustion on large extents. Round Julian day rasters to ensure
  # consistent integer-based grouping in the frequency table.
  s_round <- terra::round(start_raster)
  e_round <- terra::round(end_raster)

  # Stack layers for cross-tabulation
  stk <- c(crop_mask, s_round, e_round)
  names(stk) <- c("class_value", "start_jd", "end_jd")

  # Compute unique combinations and their frequency
  profile_df <- as.data.frame(terra::crosstab(stk, long = TRUE))

  # Filter by valid crop classes and remove NA combinations
  ...
  # Standardize types and calculate duration
  ...
  # Filter out invalid seasons (duration <= 0)
  profile_df <- profile_df[profile_df$total_days > 0L, , drop = FALSE]

  # Return standardized columns
  profile_df[, c("class_value", "start_jd", "end_jd", "total_days", "pixel_count"), drop = FALSE]
}
