...
wapor_build_season_profile_table <- function(crop_mask, start_raster, end_raster, class_values) {
  # Optimization: Using terra::crosstab(long = TRUE) is significantly faster
  # than R-level terra::values() and data.frame manipulation because it
  # counts unique combinations in the C++ backend.
  s <- terra::c(crop_mask, start_raster, end_raster)
  ct <- as.data.frame(terra::crosstab(s, long = TRUE))
  ...
  # Filter by class_values and valid combinations
  ...
  ct$start_jd <- as.integer(round(as.numeric(as.character(ct$start_jd))))
  ct$end_jd   <- as.integer(round(as.numeric(as.character(ct$end_jd))))
  ct$total_days <- ct$end_jd - ct$start_jd + 1L
  ct$pixel_count <- as.integer(ct$pixel_count)

  # Final filter for valid seasons
  ct <- ct[ct$total_days > 0, , drop = FALSE]

  # In case multiple float combinations rounded to same JD
  if (nrow(ct) > 0) {
    ct <- stats::aggregate(
