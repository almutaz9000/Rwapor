...
wapor_build_season_profile_table <- function(crop_mask, start_raster, end_raster, class_values) {
  # Optimization: Use terra::crosstab() with long=TRUE to perform combination
  # counting in the C++ backend. This is significantly more memory-efficient
  # and faster than loading all pixel values into R with terra::values().

  # Round season rasters to ensure integer Julian dates for grouping
  r_stack <- terra::c(crop_mask, terra::round(start_raster), terra::round(end_raster))
  names(r_stack) <- c("class_value", "start_jd", "end_jd")

  # Compute frequencies of unique (class, start, end) combinations
  ct <- as.data.frame(terra::crosstab(r_stack, long = TRUE))
  ...
  # Standardize column names (crosstab names depend on layer names or indices)
  names(ct) <- c("class_value", "start_jd", "end_jd", "pixel_count")

  # Filter by class values and valid pixel counts
  ...
  ct <- ct[ct$class_value %in% as.integer(class_values) & ct$pixel_count > 0, ]
  ...
  ct <- ct[ct$total_days > 0, ]

  if (nrow(ct) == 0) return(ct)

  # Sum pixel counts for duplicate combinations created by rounding
  # (though usually handled by crosstab if inputs were already near-integers)
  stats::aggregate(
